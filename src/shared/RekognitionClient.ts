...
export default class {
   ...
   */
  async detectFaces(img: string, threshold: number = 90): Promise<(AWS.Rekognition.BoundingBox|undefined)[]> {
    ...
    if (!data.FaceDetails) return [] as (AWS.Rekognition.BoundingBox|undefined)[];
    const boundingBoxes = [];
    for (let faceDetail of data.FaceDetails)
      if (faceDetail.Confidence! >= threshold) boundingBoxes.push(faceDetail.BoundingBox);
   ...
   * Add a collection.
   */
  async addCollection(collectionId: string): Promise<{id: string, faceModelVersion: string|undefined}> {
   ...
   */
  async getCollections(): Promise<{id: string, faceModelVersion: string|undefined}[]> {
    ...
    return data.CollectionIds.map((collectionId: string, i: number) => ({
      id: collectionId,
      faceModelVersion: data.FaceModelVersions ? data.FaceModelVersions[i]||undefined: undefined}));
  }
