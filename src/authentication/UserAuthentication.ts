...
/**
 * User authentication option interface.
 */
...
export default class {
   ...
   */
  public static mount(app: express.Express, options: UserAuthenticationOptions): void {
    ...
    passport.serializeUser((user: {[key: string]: any}, done) => done(null, user.id||undefined));

    // When the request is received, the user data corresponding to the ID is acquired and stored in req.user.
    passport.deserializeUser(async (id, done) => {
      const user = <{[key: string]: string}|null> await options.model.findOne({where: {id}, raw: true});
